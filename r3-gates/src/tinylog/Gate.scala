...
abstract class Gate() {
   ...
   * */
  def inputElementSupport: Set[InputElement] = {
    this.support.filter(_.isInstanceOf[InputElement]).map(_.asInstanceOf[InputElement])
  }
}
...
class NotGate(in: Gate) extends Gate() {
  ...
  def support: Set[Gate] = in.support ++ Set(this)
}
...
class OrGate(in1: Gate, in2: Gate) extends Gate() {
  ...
  def depth: Int = {
    if (in1.depth > in2.depth) in1.depth + 1 else in2.depth + 1
  }
  def support: Set[Gate] = in1.support ++ in2.support ++ Set(this)
}
...
class XorGate(in1: Gate, in2: Gate) extends Gate() {
  def value = (in1.value || in2.value) && !(in1.value && in2.value)
  def depth: Int = {
    if (in1.depth > in2.depth) in1.depth + 2 else in2.depth + 2
  }
  def support: Set[Gate] = in1.support ++ in2.support ++ Set(this)
}
...
class MajorityGate(in1: Gate, in2: Gate, in3: Gate) extends Gate() {
  def value = ( (!in1.value && in2.value && in3.value) ||
      (in1.value && !in2.value && in3.value) || 
      (in1.value && in2.value && !in3.value) ||
      (in1.value && in2.value && in3.value) )
      
  def depth: Int = {
    if (in1.depth > in2.depth & in1.depth > in3.depth) in1.depth + 3
    else if (in2.depth > in1.depth & in2.depth > in3.depth) in2.depth + 3
    else in3.depth + 3
  }
  def support: Set[Gate] = in1.support ++ in2.support ++ in3.support ++ Set(this)
}
