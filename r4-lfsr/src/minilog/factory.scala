...
object factory {
  ...
  def buildRotator(state: Bus): Bus = {
    ...
    for (i <- 0 until state.length) {
      if (i + 1 < state.length) state(i).buildFeedback(state(i + 1))
      else state(i).buildFeedback(state(0))
    }
    state(0).set(true)
    state
  ...
  def build5BitLFSR(state: Bus): Bus = {
    ...
    for (i <- 0 until state.length) {
      if (i + 1 < state.length) state(i).buildFeedback(state(i + 1))
      else state(i).buildFeedback(state(0) ^^ state(2))
    }
    state(0).set(true)
    state
  ...
  def buildLFSR(taps: List[Int], state: Bus): Bus = {
    ...
    if (taps.isEmpty) buildRotator(state)
    else {
      for (i <- 0 until state.length) {
        if (i + 1 < state.length) state(i).buildFeedback(state(i + 1))
        else {
          var x = state(0)
          for (j <- taps) {
            state(i).buildFeedback(x ^^ state(j))
            x = x ^^ state(j)
          }
        }
      }
