...
package object futures {
  ...
  /*
   * Future-objects are created with the method 'Future.apply' 
   * (or in short form, just 'Future') that takes as parameter 
   * a function that returns a value (or fails by throwing 
   ...
   *
   */
  def incrementF(z: Future[Int]): Future[Int] = z.map( _ + 1 )
   ...
   *
   */
  def sumF(x: Future[Int], y: Future[Int]): Future[Int] = (x zip y).map( i => i._1 + i._2 )
  ...
  object ServicesF {
    ...
    def B(x: Future[Int]): Future[Int] = { x.map( _ * 2 ) }
    def C(x: Future[Int]): Future[Int] = { x.map( _ + 1 ) }
    def D(x: Future[Int]): Future[Int] = { x.map( _ * (-1) ) }
    def E(x: Future[Int]): Future[Int] = { x.map( _ * 3 ) }
    def P(x: Future[Int], y: Future[Int]): Future[Int] = { (x zip y).map( i => i._1 + i._2 ) }
    def Q(x: Future[Int], y: Future[Int]): Future[Int] = { (x zip y).map( i => i._1 * i._2 ) }
    }
  ...
  def eitherF(x: Future[Int], y: Future[Int]): Future[Int] = x.recoverWith{case a: Exception => y }
  ...
  def zeroOnArithmeticException(x: Future[Int]): Future[Int] = x.recover {case a: ArithmeticException => 0}
}
