...
object factory {
  ...
  def buildEqual(bus1: Bus, bus2: Bus): Gate = {
    ...
    (bus1 zip bus2).map(x => (x._1 && x._2) || (!x._1 && !x._2)).reduceLeft(_ && _)
  }
   ...
   *      gate g(l-1) with l == bus1.legth == bus2.length, we are done:
  ...
  def buildGategi(b1i: Gate, b2i: Gate, giminus1: Gate): Gate = {
    (!b1i && !b2i && giminus1) || (!b1i && b2i && !giminus1) || (!b1i && b2i && giminus1) || (b1i && b2i && giminus1)
  }
  ...
  def buildUnsignedLess(bus1: Bus, bus2: Bus): Gate = {
    ...
    var giminus1 = Gate.False
    for (i <- 0 until bus1.length) {
       giminus1 = buildGategi(bus1(i), bus2(i), giminus1)
    }
    giminus1
  }
   ...
   *
   */
  def buildUnsignedLessOrEqual(bus1: Bus, bus2: Bus): Gate = {
    buildUnsignedLess(bus1, bus2) || buildEqual(bus1, bus2)
  }
  ...
  def buildIncrementer(bus: Bus): Bus = {
    ...
    var carry_in = Gate.True //Initial carry is 1, because we want to add 1 to the bus
    var ss = new Array[Gate](bus.length)
    for (i <- 0 until bus.length) {
      val (carry_out, sum) = buildHalfAdder(bus(i), carry_in)
      carry_in = carry_out
      ss(i) = sum
    }
    new Bus(ss)
  ...
  def buildCountTrueBits(bus: Bus): Bus = {
    ...
    val n = log2ceil(bus.length)
    if (bus.length == 1) {
      bus
    }
    else if (bus.length == 2) {
      val (c_out, sum) = buildHalfAdder(bus(0), bus(1))
      Bus(sum, c_out)
    }
    else if (bus.length == 3) {
      val (c_out, sum) = buildFullAdder(bus(0), bus(1), bus(2))
      Bus(sum, c_out)
    }
    else {
      val aa = buildCountTrueBits(bus.drop(bus.length / 2))
      val bb = buildCountTrueBits(bus.take(bus.length / 2))
      buildAdderNoOverflow(aa, bb).take(n)
    }
  }
  
  def incrementer(bus: Bus, carryin: Gate): Bus = {
    var carry = carryin
    var ss = new Array[Gate](bus.length)
    for(i <- 0 until bus.length){
      val(carryout, sum) = buildHalfAdder(bus(i), carry)
      carry = carryout
      ss(i) = sum
    }
  ...
  def buildCountFalseBits(bus: Bus): Bus = {
    require(bus.length > 0, "Can not build a true bit counter on an empty bus")
    val newlength = log2ceil(bus.length)
    var ss = Bus.falses(newlength)
    for(i <- 0 until bus.length){ //jos 0: +1 (build increment)
      var asd = incrementer(ss, !bus(i))
      ss = asd
  ...
  def buildMajorityTester(bus: Bus): Gate = {
    ...
    buildUnsignedLess(buildCountFalseBits(bus), buildCountTrueBits(bus))
  }
  /**
  ...
  def buildMultiplier(bus1: Bus, bus2: Bus): Bus = {
    ...
    var bus = Bus.falses(bus1.length)
    for (i <- 0 to bus2.length - 1) {
      val b = Bus.falses(i)
      val row = b ++ (bus1 && bus2(i))
      bus = buildAdderNoOverflow(row, bus)
    }
    bus
  }
}
