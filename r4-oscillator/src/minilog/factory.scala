...
object factory {
  ...
  def buildOscillatorPeriod4(host: Circuit): Gate = { 
    ...
    t(0).buildFeedback(t(1))
    t(1).buildFeedback(t(2))
    t(2).buildFeedback(t(3))
    t(3).buildFeedback(t(0))
  ...
  def buildOscillator(host: Circuit, period: Int): Gate = {
    ...
    val t = host.inputs(period)
    
    for (i <- 0 until period) {
      if (i + 1 < period) t(i).buildFeedback(t(i + 1)) else t(i).buildFeedback(t(0))
    }
