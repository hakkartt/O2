...
class UnitTests {
  ...
  @Test def testN200() {
    /* This test should finish in less than two seconds on a good desktop machine with 3.1GHz CPU or similar*/
    val n = 150000
    val result = (Vector.tabulate(n)(2*_) ++ Vector.tabulate(n)(2*_+1)).reverse
    val str = "ba" * n
    assertEquals("testN200() (performance) failed", suffixArray(str), result)
  }
}
