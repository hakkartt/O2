...
abstract class LinkedList[A]() {
  ...
  final def count(p: A => Boolean): Int = {
    @tailrec def inner(list: LinkedList[A], r: Int): Int = {
      list match {
        case Nil() => return r
        case Cons(h, t) => if (p(h)) inner(t, r + 1) else inner(t, r)
      }
    }
    inner(this, 0)
  }
  ...
  final def filterNot(p: A => Boolean): LinkedList[A] = {
    @tailrec def inner(list: LinkedList[A], r: LinkedList[A]): LinkedList[A] = {
      list match {
        case Nil() => return r.reverse
        case Cons(h, t) => if (p(h)) inner(t, r) else inner(t, Cons(h, r))
      }
    }
    inner(this, Nil())
  }
   ...
   */
  final def last: A = {
    @tailrec def inner(list: LinkedList[A]): A = {
      list match {
        case Nil() => throw new java.util.NoSuchElementException
        case Cons(h, t) => if (t == Nil()) h else inner(t)
      }
    }
    inner(this)
  }
  ...
  final def drop(n: Int): LinkedList[A] = {
    @tailrec def inner(list: LinkedList[A], count: Int): LinkedList[A] = {
      list match {
        case Nil() => list
        case Cons(h, t) => if (count == n) list else inner(t, count + 1)
      }
    }
    inner(this, 0)
  }
  ...
  final def map[B](f: A => B): LinkedList[B] = {
    @tailrec def inner(list: LinkedList[A], r: LinkedList[B]): LinkedList[B] = {
      list match {
        case Nil() => r.reverse
        case Cons(h, t) => inner(t, Cons( f(h), r ))
      }
    }
    inner(this, Nil())
  }
  ...
  final def foldLeft[B](start: B)(f: (B, A) => B): B = {
     @tailrec def inner(list: LinkedList[A], r: B): B = {
      list match {
        case Nil() => r
        case Cons(h, t) => inner(t, f(r, h))
      }
    }
    inner(this, start)
  }
  ...
  final def splitAt(n: Int): (LinkedList[A], LinkedList[A]) = {
    @tailrec def inner(a: LinkedList[A], b: LinkedList[A], cnt: Int): (LinkedList[A], LinkedList[A]) = {
      b match {
        case Nil() => (a.reverse, b)
        case Cons(h, t) => if (cnt < n) {
          if (t == Nil()) { inner(Cons(h, a), Nil(), cnt + 1) }
          else { inner(Cons(h, a), Cons(t.head, t.tail), cnt + 1) }
        } 
        else (a.reverse, b)
      }
    }
